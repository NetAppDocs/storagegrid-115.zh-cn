...
----
最后、 `audit-sum` 工具还可以接受来自管道的输入、这样您可以使用筛选和预处理输入 `grep` 命令或其他方式。例如：
...
NOTE: 此工具不接受将压缩文件作为管道输入。要处理压缩的文件、请以命令行参数形式提供其文件名、或者使用 `zcat` 用于先解压缩文件的工具。例如：
...
.. 要显示速度最慢的10个检索操作、请使用grep命令仅选择SGET消息并添加长输出选项 (`-l`)以包括对象路径： `grep SGET audit.log | audit-sum -l`
+
...
.. 要查看更多详细信息、请使用 `audit-explain` 用于查看该小时内所有SGET操作的工具：
+
[listing]
----
