...
summary: 当S3客户端发出后对象还原请求时、系统会请求将对象从AWS Glacier存储还原到云存储池。如果事务成功，服务器将发出此消息。 
---
= SPOS ： S3 POST
:allow-uri-read: 
:icons: font
...
[role="lead"]
当S3客户端发出后对象还原请求时、系统会请求将对象从AWS Glacier存储还原到云存储池。如果事务成功，服务器将发出此消息。
