...
* 如果您使用共享存储来支持 StorageGRID 节点，或者已将部分或全部磁盘驱动器或 SSD 从故障节点移至替代节点，则必须重新建立原始主机上的相同存储映射。例如、如果您在中使用了WWID和别名 `/etc/multipath.conf` 按照安装说明中的建议、请务必在中使用相同的别名/WWID对 `/etc/multipath.conf` 在替代主机上。
