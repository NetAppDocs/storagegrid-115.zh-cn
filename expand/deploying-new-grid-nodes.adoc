...
.步骤
. 将新网格节点部署为虚拟机、并将其连接到一个或多个StorageGRID 网络。
+
部署节点时，您可以选择重新映射节点端口或增加 CPU 或内存设置。

+
link:../vmware/deploying-storagegrid-node-as-virtual-machine.html["将StorageGRID 节点部署为虚拟机"]

. 部署所有新的VMware节点后、请返回以下说明以执行扩展操作步骤。
+
...
* 您已使用StorageGRID 设备安装程序完成设备安装和维护说明中的所有"`配置硬件`"步骤。
+
...
* 此替代设备上的StorageGRID 设备安装程序版本与您的StorageGRID 系统软件版本匹配。(如果版本不匹配、则必须升级StorageGRID 设备安装程序固件。)
+
有关说明、请参见设备安装和维护说明。
...
. 如果需要手动访问监控器安装页面、请单击菜单栏中的*监控器安装*。
+
