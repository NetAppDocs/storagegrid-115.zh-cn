...
使用的格式为： `<network type>/<protocol>/<default port used by grid node>/<new port>`、其中网络类型为网格、管理员或客户端、协议为TCP或UDP。
...
使用的格式为： `<network type>/<protocol:>/<remapped port >/<default port used by grid node>`、其中网络类型为网格、管理员或客户端、协议为TCP或UDP。
